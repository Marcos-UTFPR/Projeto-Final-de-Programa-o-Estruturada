...
int main(void) {
  ...
  carrosAtendidos1 = (struct TCarro*)calloc(0,sizeof(struct TCarro));
  ...
  filaAtual = (struct TCarro*)calloc(tamanhofila,sizeof(struct TCarro));
    ...
    switch (opcaomenu){
      case 1: //Adicionar um carro na fila
        if (combustivel == 0){
          ...
          if (fila < tamanhofila){
            fflush(stdin); //Tem que pegar a funçao de flushin nos slides para substituir isso
            fgets(filaAtual[fila].modelo,20,stdin);
            printf("\nInforme o modelo do carro: ");
            fflush(stdin);
            fgets(filaAtual[fila].modelo,20,stdin); //Por alguma razão, isso dá certo
            ...
            fflush(stdin);
            fgets(filaAtual[fila].cor,20,stdin);
            printf("\nInforme a cor do carro: ");
            fflush(stdin);
            fgets(filaAtual[fila].cor,20,stdin);
            filaAtual[fila].cor[strcspn(filaAtual[fila].cor,"\n")]=0; //Por alguma razão, isso dá certo também
      ...
      case 2: //Abastecimento
        if (fila != 0){
          ...
          carrosAtendidos1[carrosatendidos] = filaAtual[0];
          ...
          if (combustivel == 0){
            ...
            fila = 0; //Zera a fila quando acaba o combustível, mas, apesar de não zerar o vetor que gerencia a fila, não tem mais como interagir com tal vetor, pois as ações são bloqueadas
          }    
        }
        else {
